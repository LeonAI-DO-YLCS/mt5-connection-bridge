...
def test_cancel_order_success(client, auth_headers, mock_mt5_submit, completed_future_factory):
    from app.models.trade import TradeResponse
    from app.main import settings
    settings.execution_enabled = True
    mock_mt5_submit.return_value = completed_future_factory((TradeResponse(success=True, ticket_id=12345, error=None), "cancelled"))

    response = client.delete("/orders/12345", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
    assert data["ticket_id"] == 12345

def test_cancel_order_execution_disabled(client, auth_headers, monkeypatch):
    from app.main import settings
...
def test_cancel_order_connection_error(client, auth_headers, mock_mt5_submit):
    from app.main import settings
    settings.execution_enabled = True
